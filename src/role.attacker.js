...
var roleAttacker = {
	run : function(creep, utils)
	{
		...
		if (creep.memory.flag == null)
		{
			var ctr = 0;
			while (ctr < 30
					&& (creep.memory.flag == null || Game.flags[creep.memory.flag] == null))
			{
				ctr++;
				creep.memory.flag = 'AttackRoom'
...
function chooseHostile(closestHostiles)
{
	...
	for (h in closestHostiles)
	{
		...
		if (hostile.owner.username != 'Cokezero'
				...
				&& hostile.structureType != STRUCTURE_POWER_BANK
				&& hostile.structureType != STRUCTURE_TERMINAL
				&& hostile.structureType != STRUCTURE_WALL)
		{
